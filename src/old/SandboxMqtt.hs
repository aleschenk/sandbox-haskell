...
main = do
  ...
  mc <-
    connectURI
      mqttConfig
        { _lwt =
            Just $
              (mkLWT "tmp/haskquit" "bye for now" False)
                { _willProps = [PropUserProperty "lwt" "prop"]
                },
          _msgCB = SimpleCallback showme,
          _protocol = Protocol50,
          _connProps =
            [ PropReceiveMaximum 65535,
  ...
  publishq
    mc
    "tmp/q1"
    "this message is at QoS 1"
    True
    QoS1
    [ PropUserProperty "hi" "there 1",
  ...
  publishq
    mc
    "tmp/q2"
    "this message is at QoS 2"
    True
    QoS2
    [ PropUserProperty "hi" "there 2",
      PropMessageExpiryInterval 30,
      PropContentType "text/plain"
    ]
  putStrLn "Published!"
